@extends('base')
@section('big_title')
    <span class="bi-person-fill" style="color: peru"> INFORMATION DE PROFILE</span>
@endsection
...
@section('content')
    <section class="shadow-lg p-4">
        <form method="post" action="{{ route('user.update_profile', $user->id) }}" class="mt-6 space-y-6"
            ...
            <div class="mb-3">
                <input id="id_photo" name="photo" accept=".jpeg, .jpg, .png"
                       onchange="file_validate(this)" value="{{ $user->photo }}" type="file" class="form-control"/>
