            ...
            <div class="btn-group dropdown" style="float: right;">
                ...
                <ul class="dropdown-menu p-2" role="menu" style="background-color: #ffffff; border: 1px solid blue">
                    <li>
                        ...
                        <a class="dropdown-item btn btn-outline-secondary perso" href="{{ route('report.releve_des_transactions_caisse_solidarite', [$avec->id, $projet->id]) }}"><span class="bi-file-earmark-excel-fill text-warning"> rélevé caisse solidarité</span></a>
                        <a class="dropdown-item btn btn-outline-secondary perso" href="{{ route('report.rapport_transactions_de_avec', [$avec->id, $projet->id]) }}"><span class="bi-file-earmark-excel-fill text-primary"> rapports des transactions</span></a>
                        <a class="dropdown-item btn btn-outline-secondary perso" href="{{ route('report.situation_generale_de_avec', [$avec->id, $projet->id]) }}"><span class="bi-search"> situation générale de l'avec</span></a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route("projet.supprimer_un_membre") }}" method="post">
                        @csrf
                        @method("delete")
                        ...
                        <table id="multi-filter-select" class="display table-sm w-100 table-striped table-hover">
                            ...
                            @foreach($membres as $membre)
                                <tr class="ligne ln-pdf">
                                    ...
                                    <td class="d-flex">
                                        <a class="btn-sm text-success" href="{{ route('projet.afficher_membre', $membre->id) }}"><span class="bi-eye"></span></a>
                                        @if($projet->statut === "en cours" || $projet->statut === "en attente")
                                            @if(\Carbon\Carbon::parse($membre->created_at)->diffInDays() <= 7 || $membre->statut === "abandon")
